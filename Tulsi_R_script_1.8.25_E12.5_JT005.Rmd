...
This dataset is JT005 --. These are E12.5 mouse proprioceptor cells. In this early datasets (pre-P0), the reporter gene used was Ntrk3. Cells were then dissected out of the DRG based on expression of a TdTomato reporter.

```{r setup}

knitr::opts_knit$set(root.dir = "C:\\Users\\Jordan Henry\\Proprioception Datasets\\JT005_E12.5")
...
joint_calling_image = "JT005_Joint_Cell_Calling.png"
...
--------
Note[2]: I'm reading that 'validate.fragments=FALSE' should only be used
...
At E12.5 cells, our proprioceptors *should* express Parvb and Runx3 if they are intact cells. (Add Ntrk3 and Etv1, to be extra sure.) --All of this should be true from at least P0. Before P0 thereis no Whirlin.
...
**Based on expression of on-target(Ntrk3, Etv1, etc.) and off-target markers (Ntrk2 & Ret), I think it makes sense to select 0,1,2,8 & 13, plus 4 & 12 as our proprioceptor cluster. -- We exclude 3,5,6,11 & 15 for expressing Ntrk2, which is an off-target marker. We also exclude the other groupfor not expressing the cannonical markers.**
...
PSN.proprio <- subset(x = PSN, idents = c("0", "1","2", "4","8", "12", "13"))
#Create another Seurat object to house the "off-target" cells.
PSN.offtarget <- subset(x = PSN, idents = c("3", "5", "6", "7", "9", "10", "11", "14", "15"))
```
...
Thinking about the concept that dimensionality reduction is destined to produce some distortions from reality, I'm not sure how to interpret these results. --But it does seem clear that the bulk of the group-level variation is captured in PCs 1-5. 

For now, we utilize all 20 PCs for downstream projections, to avoid leaving any information on the table.
...
At E12.5, we find very little Nxph1 expression. The expression of Csmd3 is more concentrated. Perhaps we can use this to correlate back to the identity of these particular cells, but perhaps Csmd3 expression is only restricted to group 1b afferents at P56?
...
Let's reproject groups 0, 1, 2, 8 &13.

#Comparing groups 0, 1, 2, 8, 13.

```{r}
PSN.prop_ <- subset(x = PSN, idents = c(0, 1, 2, 8, 13))
```
...
```
Again, let's preserve the same labels. --Also using only the first 5 PCs.
...
## Comparing groups 4 and 12.

```{r}
#Create a new proprioceptor Seurat Object with just the clusters that match the proprioceptor profile
PSN.prop_ <- subset(x = PSN, idents = c("4", "12"))
```
